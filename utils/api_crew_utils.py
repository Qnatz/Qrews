...
# AGENT_CONFIG_PATH = os.path.join(os.path.dirname(__file__), 'agent_config.json') # OLD PATH
# Corrected path: from utils/ up to root, then down to configs/
AGENT_CONFIG_PATH = os.path.normpath(os.path.join(os.path.dirname(__file__), '..', 'configs', 'api_agent_config.json')) # CORRECTED
...
class SubAgentWrapper:
    def __init__(self, sub_agent_name: str, output_model: Type[T], logger: Optional[LoggerPlaceholder] = None):
        ...
        try:
            from prompts.api_designer_prompts import get_sub_agent_prompt as ext_get_sub_agent_prompt # CORRECTED
            self.get_prompt_function = ext_get_sub_agent_prompt
        except ImportError as e:
            self.logger.log(f"Could not import get_sub_agent_prompt from prompts.api_designer_prompts: {e}. Ensure structure is correct.", level="ERROR")
